    ...
    <div className="relative group w-fit">
      ...
      <div className="flex items-center space-x-8 bg-white px-6 py-4 rounded-t-lg border-b border-gray-100">
        ...
        >
          ...
          {activeTab === 'energy' && (
            <span className="absolute bottom-[-17px] left-0 w-full h-[2px] bg-[#D80621]"></span>
          )}
        </button>
              ...
              {activeSubItems.map((item, index) => (
                <li key={index}>
                  <Link 
                    ...
                    className={`block text-sm transition-colors ${ item === 'Electrical Equipment' ? 'text-[#D80621] font-medium' : 'text-[#D80621] hover:text-[#D80621]' }`}
                  >
                    {item}
                  </Link>
                </li>
